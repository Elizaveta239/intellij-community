...
class ThreadingLogger:
    ...
    def send_message(self, time, thread_id, type, event, file, line):
            ...
            if write_log:
                ...
                if back_base not in DONT_TRACE_THREADING:
                    ...
                    if isinstance(self_obj, threading.Thread) and method_name in THREAD_METHODS:
                        ...
                        self.send_message(event_time, thread_id, "thread", frame.f_code.co_name, back.f_code.co_filename, back.f_lineno)
                        print(event_time, thread_id, back_base, back.f_lineno, frame.f_code.co_name)
                    elif self_obj.__class__ == LockWrapper:
                        if DictContains(frame.f_locals, "attr") and frame.f_locals["attr"] in LOCK_METHODS:
                            thread_id = GetThreadId(self_obj)
                            self.send_message(event_time, GetThreadId(t), "lock", frame.f_locals["attr"], back.f_code.co_filename, back.f_lineno)
                            print(event_time, GetThreadId(t), back_base, back.f_lineno, frame.f_locals["attr"])
                        elif method_name in LOCK_CONTEXT_MGR:
                            thread_id = GetThreadId(self_obj)
                            self.send_message(event_time, GetThreadId(t), "lock", frame.f_code.co_name, back.f_code.co_filename, back.f_lineno)
                            print(event_time, GetThreadId(t), back_base, back.f_lineno, frame.f_code.co_name)
        except e:
